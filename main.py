...
# ==========================================================
# ESTRUCTURA DE PESTAÑAS Y DISPONIBILIDAD DE CONTENIDO
# ==========================================================
tab_resumen, tab_graficos, tab_evolucion, tab_distribucion, tab_detalles = st.tabs([
    "📋 Resumen de Fondos", "📈 Gráficos de Evolución", "📊 Histórico", "🍩 Distribución", "🔍 Detalles"
])

with tab_resumen:
    # Formatear el dataframe final para mandarlo a la tabla financiera premium
    df_tabla_final = final.copy()

    # Formatear números para visualización limpia en columnas estáticas
    columnas_moneda = ["Invertido", "Valor actual", "Ganancia", "Precio VL"]
    for col in columnas_moneda:
        if col in df_tabla_final.columns:
            df_tabla_final[col] = df_tabla_final[col].apply(lambda x: f"{x:,.2f} €" if pd.notnull(x) else "")

    columnas_porcentaje = ["Rentabilidad (%)", "1 día (%)", "7 días (%)", "1 mes (%)"]
    for col in columnas_porcentaje:
        if col in df_tabla_final.columns:
            df_tabla_final[col] = df_tabla_final[col].apply(lambda x: f"{x:+.2f} %" if pd.notnull(x) else "")

    if "Última actualización" in df_tabla_final.columns:
        df_tabla_final["Última actualización"] = pd.to_datetime(df_tabla_final["Última actualización"]).dt.strftime('%d/%m/%Y')

    # Renderizar tabla final estilizada
    render_financial_table(
        df_tabla_final, 
        cols_color_render=["Ganancia", "Rentabilidad (%)", "1 día (%)", "7 días (%)", "1 mes (%)"]
    )

with tab_graficos:
    st.info("Espacio reservado para las gráficas interactivas de evolución de fondos.")

with tab_evolucion:
    st.info("Espacio reservado para la tabla histórica detallada del portafolio.")

with tab_distribucion:
    st.info("Espacio reservado para la asignación de activos y gráficos de tarta.")

with tab_detalles:
    st.info("Espacio reservado para auditorías de órdenes detalladas e ISINs.")