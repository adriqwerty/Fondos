...
# ==========================================================
# 🌌 INTERFAZ COMPLETA EN MODO OSCURO TRABAJADA (CSS MEJORADO)
# ==========================================================
st.set_page_config(page_title="Inversiones", layout="wide", initial_sidebar_state="expanded")

# Inyección de CSS global (Sidebar, Inputs, Selectores y Contenedores)
st.markdown("""
    <style>
    ...
    div[data-baseweb="select"] * {
        color: #f8fafc !important; /* Texto del selector en blanco */
    ...
    ul[data-baseweb="menu"] li:hover {
        background-color: #334155 !important; /* Color al pasar el ratón por las opciones */
    }
...
st.markdown("""
    ...
    /* Eliminar borde de la última fila para que no quede un espacio extraño abajo */
...
def render_financial_table(df_styled, cols_color_render=None):
    """
    Transforma el DataFrame eliminando registros fantasmas o vacíos 
    para ajustar el tamaño exacto en pantalla.
    """
    # Eliminar filas donde todos los elementos sean NaN o strings vacíos antes de renderizar
    ...
    for _, row in df_clean.iterrows():
        # Validar si la fila tiene contenido real para evitar pintar la "media fila vacía"
        ...
        for col in df_clean.columns:
            ...
            # Si el valor es nulo de Pandas, lo dejamos limpio
...
# TAB 1: RESUMEN DE FONDOS (Limpiada de filas vacías)
with tab_resumen:
