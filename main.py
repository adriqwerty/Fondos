...
# buscar último valor DIFERENTE al actual
prev_value = None

for v in reversed(portfolio["valor"].iloc[:-1].values):
...
portfolio["1d (%)"] = (
    ((last_value - prev_value) / last_value) * 100
    if prev_value is not None and last_value != 0
    else None
)
